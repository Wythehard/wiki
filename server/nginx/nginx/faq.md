...
```
在 /etc/nginx/nginx.conf  查看 error_log
找到对应错误日志目录 /var/log/nginx
...
\#文件不存在转404  try_files    $uri = 404;

kill -HUP pid  向 ngixn 发送重启信号 重新加载配置 等价于 ./nginx -s  （reload|stop）


### nginx location 配置匹配问题

```
refer:https://moonbingbing.gitbooks.io/openresty-best-practices/ngx/nginx_local_pcre.html
